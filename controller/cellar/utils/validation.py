...
def _validate_n_clusters(n_clusters, h):
    if n_clusters >= h:
        raise InvalidArgument("Number of clusters needs to be less than "
                                    "the number of samples.")
    if n_clusters < 2:
        raise InvalidArgument("Number of clusters needs to be greater "
                                    "than 1.")
...
def _validate_n_jobs(n_jobs):
    ...
    if isinstance(n_jobs, int):
        if n_jobs < -1:
            ...
            raise InvalidArgument("Number of jobs is too high.")
        elif n_jobs == 0:
            raise InvalidArgument("Number of jobs is 0.")
    elif n_jobs is None:
        n_jobs = 1
